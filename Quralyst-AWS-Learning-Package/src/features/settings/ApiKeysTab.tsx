...
import type { ApiKeyTestResult, OrgKeyName, PreferencesData } from '@/types';
...
export default function ApiKeysTab() {
  ...
  const configured = useMemo(
    () => prefs?.apiKeyStatus ?? ({} as Record<OrgKeyName, boolean>),
    [prefs],
  );
  const source = prefs?.apiKeySource ?? {};

  const runTest = async () => {
    setBusy('test');
    try {
      const data = await preferencesService.testApiKeys();
      if (data.success) {
        setResults(data.testResults);
        const keys = Object.keys(data.testResults) as OrgKeyName[];
        const valid = keys.filter((k) => data.testResults[k].status === 'valid').length;
        ...
        keys.forEach((k) => {
          const name = k
            ...
            .replace(/api key/i, '')
            .trim();
          summary += `${name.charAt(0).toUpperCase()}${name.slice(1)}: ${data.testResults[k].message}\n`;
        });
        success('API Keys Test Complete', summary, false);
      } else {
        error('API Keys Test Failed', data.message || 'Unknown error occurred');
      }
    } catch {
  ...
  const testSingleKey = async (keyName: OrgKeyName, keyValue?: string) => {
    setBusy('test');
    try {
      const data = await preferencesService.testSingleApiKey(keyName, keyValue);
      if (data.success) {
        if (!keyValue) {
          setResults((prev) => ({ ...prev, [keyName]: data.testResults[keyName] }));
        }
        const res = data.testResults[keyName];
        if (res.status === 'valid') {
          success('Test Successful', res.message);
        } else {
          error('Test Failed', res.message);
        }
      } else {
        error('Test Failed', data.message || 'Unknown error occurred');
      }
    } catch {
  ...
  const updateKey = async (keyName: OrgKeyName) => {
    ...
    setBusy('update');
    try {
      const data = await preferencesService.updateApiKeys({ [keyName]: v });
      if (data.success) {
        setPrefs((p) =>
          p
            ? {
                ...p,
                apiKeyStatus: { ...p.apiKeyStatus, [keyName]: true },
                apiKeySource: { ...p.apiKeySource, [keyName]: 'user' },
              }
            : p,
        );
        setResults((r) => {
        ...
        success('API Key Saved', data.message || 'API key updated successfully.');
        setExpandedKey(null);
        setInputValue('');
        // Only verify the key that was just saved — full suite is "Test All API Keys".
        setTimeout(() => void testSingleKey(keyName), 600);
      } else {
        error('Update Failed', data.message || 'Failed to update API key.');
      }
    } catch {
  ...
  const clearKey = async (keyName: OrgKeyName) => {
    const ok = await confirm({
      title: 'Clear API Key',
      message: 'Are you sure you want to clear this API key?',
    ...
    setBusy('clear');
    try {
      const payload: Partial<Record<OrgKeyName, string>> = { [keyName]: '' };
      const data = await preferencesService.updateApiKeys(payload);
      if (data.success) {
        setPrefs((p) => {
          if (!p) return p;
          const nextSource = { ...p.apiKeySource };
          delete nextSource[keyName];
          return {
            ...p,
            apiKeyStatus: { ...p.apiKeyStatus, [keyName]: false },
            apiKeySource: nextSource,
          };
        });
        setResults((r) => {
          const next = { ...r };
        ...
        success('API Key Cleared', data.message || 'The API key has been removed.');
      } else {
        error('Clear Failed', data.message || 'Failed to clear API key.');
      }
    } catch {
  ...
  const clearAllKeys = async () => {
    const ok = await confirm({
      ...
      message: 'Are you sure you want to clear all of your API keys? This cannot be undone.',
    ...
    setBusy('clear');
    try {
      const data = await preferencesService.clearApiKeys();
      if (data.success) {
        setPrefs((p) => {
          if (!p) return p;
          const apiKeyStatus = { ...p.apiKeyStatus };
          (Object.keys(apiKeyStatus) as OrgKeyName[]).forEach((k) => {
            apiKeyStatus[k] = false;
          });
          return { ...p, apiKeyStatus, apiKeySource: {} };
        });
        setResults({});
        setExpandedKey(null);
        setInputValue('');
        success('API Keys Cleared', data.message || 'All API keys have been cleared.');
      } else {
        error('Clear Failed', data.message || 'Failed to clear API keys.');
      }
    } catch {
  ...
  return (
    <div className="org-section settings-api-keys-section">
      <div className="org-section-header">
        <div>
          <h2 className="org-section-title">Your API keys</h2>
          <p className="org-section-sub">
            Used for your own runs. Same layout and testing as Organization Settings. Your keys take
            precedence over org defaults when set.
          </p>
        </div>
                        ...
                        <div className="api-key-row">
                              ...
                              <div className="api-key-header">
                                ...
                                <div className="api-key-badges ms-2">
                                  ...
                                  {src === 'org' ? (
                                    ...
                                    <span className="settings-badge settings-badge--user">
                                      Your key
                                    </span>
                                  ) : null}
                                </div>
                          ...
                          <div className="api-key-actions d-flex align-items-center">
                            <button
                              ...
                              disabled={busy === 'test' || (!configured[kn] && src !== 'user')}
                            >
                            ...
                            <button
                              ...
                              onClick={() => clearKey(kn)}
                              title="Clear Key"
                              disabled={busy === 'clear' || (!configured[kn] && src !== 'user')}
                            >
                        ...
                        {isExpanded && (
                          <div className="api-key-expand-content">
                            ...
                            <div className="small text-muted">
                              ...
                              Keys are stored encrypted per-user and used only for your runs. They
                              are never shared or logged.
                            </div>
                          </div>
                        )}
                      </div>
