...
- **Secrets in client code.** Any token/key/password in `src/`, env usage beyond the documented public
  `VITE_*` vars, secrets baked into the bundle. (secretlint runs pre-commit; verify nothing slipped in.)
...
- **Auth.** Session is an httpOnly cookie with **CSRF double-submit** — verify tokens are echoed on
  mutations and that **no auth token is stored in `localStorage`/`sessionStorage`**. Check idle-timeout
  and permission-map (`can()`) gating on protected routes.
...
- **Source maps & logging.** `.map` files must not be deployed publicly; no PII/secrets in shipped logs
  (`/api/logs`) or Sentry payloads.
